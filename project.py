...
while(1):
	...
	contour,hierarchy=cv2.findContours(red_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	...
	contour,hierarchy=cv2.findContours(green_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	for pic, contour in enumerate(contour):
		area=cv2.contourArea(contour)
		if(area>300):
			x,y,w,h=cv2.boundingRect(contour)
